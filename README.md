...
基于 ASP.NET Web Forms 4.8 的高校学生宿舍管理系统，支持学生端（网页端优先）和管理端（网页端优先）。
...
### 管理端
- **仪表盘**：宿舍统计、入住率、报修趋势图
- **宿舍分配**：按学院/专业/年级筛选学生，手动分配/释放床位，分页浏览
- **报修管理**：工单详情、指派维修人员、完成/驳回，现场照片查看
- **选宿批次**：创建/编辑批次、限定学院/专业/年级、选择宿舍范围、自动判断状态
...
├── App_Code/           # 业务逻辑层（BLL）
│   ├── DBHelper.cs     # 数据库访问封装
│   ├── UserBLL.cs      # 用户业务（登录、注册、重置密码）
│   ├── DormBLL.cs      # 宿舍业务 + DeptBLL（院系管理）
│   ├── RepairBLL.cs    # 报修业务
│   ├── AdminBLL.cs     # 管理员账号管理
│   ├── BatchBLL.cs     # 选宿批次业务
│   └── NoticeBLL.cs    # 通知公告业务
├── admin/              # 管理端页面（母版页 + 7 个功能页）
├── student/            # 学生端页面（母版页 + 5 个功能页）
├── css/                # 样式文件（style.css / admin.css / student.css）
├── sql/                # 数据库脚本
├── Uploads/            # 上传文件目录（avatars / repair）
├── login.aspx          # 登录页
├── register.aspx       # 注册页
├── reset-password.aspx # 忘记密码页
└── web.config          # 配置文件
```
